...
test_that("Check results (from subscreencalc)",{
  expect_equal(class(results), "SubScreenResult")
  expect(is.list(results), failure_message = "result object is not a list")
  expect_equal(results$results_total$N.of.subjects, 312)
  expect_equal(results$min_comb, 1)
  expect_equal(results$max_comb, 3)
})
