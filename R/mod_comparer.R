 ...
 plot_points_data3 <- shiny::reactive({
  ...
  output$cont_well2 <- shiny::renderUI({
    shiny::tags$head(
      shiny::tags$style(
        type = 'text/css',
        paste0(".myclass2 {background-color: ", ColorBGplot(),";}")
      )
    )
  })

  output$y1 <- shiny::renderUI({
    start_y <- names(results()$results_total)[1]
    shiny::selectInput(
      inputId = ns("y1"),
  ...
  output$YRange2 <- shiny::renderUI({
    ...
    shiny::sliderInput(
      ...
      min = roundDownNice(min(results()$sge[, input$y1], na.rm = TRUE)),
      max = roundUpNice(max(results()$sge[, input$y1], na.rm = TRUE)),
      value = c(roundDownNice(min(results()$sge[, input$y1], na.rm = TRUE)),roundUpNice(max(results()$sge[, input$y1], na.rm = TRUE))),
      step = roundUpNice((max(results()$sge[, input$y1], na.rm = TRUE) - min(results()$sge[, input$y1], na.rm = TRUE))/100)
    )

  } else {
      rg.z <- log(
        range(
          roundDownNice(min(results()$sge[, input$y1], na.rm = TRUE)),
          roundUpNice(max(results()$sge[, input$y1], na.rm = TRUE))
        )
      )
      choices <- unique(
        unlist(
  ...
  output$cont_well3 <- shiny::renderUI({
    shiny::tags$head(
      shiny::tags$style(
        type = 'text/css',
        paste0(".myclass3 {background-color: ", ColorBGplot(),";}")
      )
    )
  })
  ...
  output$y2 <- shiny::renderUI({
    shiny::selectInput(
      ...
      choices = names(results()$results_total),
      selected = names(results()$results_total)[2]
    )
  })
  ...
  output$YRange3 <- shiny::renderUI({
    ...
    shiny::sliderInput(
      ...
      min = roundDownNice(min(results()$sge[, input$y2], na.rm = TRUE)),
      max = roundUpNice(max(results()$sge[, input$y2], na.rm = TRUE)),
      value = c(roundDownNice(min(results()$sge[, input$y2], na.rm = TRUE)), roundUpNice(max(results()$sge[, input$y2], na.rm = TRUE))),
      step = roundUpNice((max(results()$sge[, input$y2], na.rm = TRUE) - min(results()$sge[, names(results()$results_total)[2]], na.rm = TRUE))/100)
    )
  } else {
      rg.z <- log(
        range(
          roundDownNice(min(results()$sge[, input$y2], na.rm = TRUE)),
          roundUpNice(max(results()$sge[, input$y2], na.rm = TRUE))
        )
      )
  ...
  mod_graph_vars2 <- shiny::callModule(
    ...
    YRange = shiny::reactive({input$YRange2}),
  ...
  mod_graph_vars3 <- shiny::callModule(
    ...
    YRange = shiny::reactive({input$YRange3}),
    XRange = shiny::reactive({NULL}),
  ...
  mod_graph_vars4 <- shiny::callModule(
    ...
    XRange = shiny::reactive({input$YRange2}),
    YRange = shiny::reactive({input$YRange3}),
    plot_type = shiny::reactive({shiny::req(input$plot_type2)}),
