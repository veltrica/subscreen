...
pseudo_contexts <- function(data, endpoint, factors) {
  ...
  if(dim(pseudo_all )[1] != 0) {
    ...
    #pseudo_all[pseudo_all$SGID %in% pseudo_fc$SGID,paste0("FCID_complete_",endpoint)] <- "Not complete"
    #pseudo_all[pseudo_all$SGID %in% pseudo_fc$SGID,paste0("FCID_incomplete_",endpoint)] <- "Incomplete"
